...
def main():
    ...
    model = model.cuda()
    output_dir = '/data/yonghao.xu/DFC2019/track1/Result/Test/'
...
#%%
    for i, sample in enumerate(tbar):
        ...
        if args.crf==False:
            for j in range(8):
                DSMoutName = name[0].replace(IMG_FILE_STR, DEPTH_FILE_STR+repr(j+1))

                DSMpred = label_mat_3[j]     
                DSMpred = convert_labels(DSMpred, toLasStandard=True)
                tifffile.imsave(os.path.join(output_dir, DSMoutName), DSMpred, compress=6)
        '''
#%%
